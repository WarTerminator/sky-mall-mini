Component<any, any, any>({
  ...
  methods: {
    ...
    _validateShowTime(_value: string, _mode: string) {
    ...
    _gotDateTimeList({ _start, _end, _type }:any) {
    ...
    _gotDayList(_year:number, _mouth:number) {
    ...
    //点击确定时触发的回调函数
    //@param {Number} ev
    selectChangeFn(ev: any) {
      const selectValues = ev.detail.value
      const rangeList = this.data.rangeList
      let dateTime = ''
      selectValues.map((el:any, index:number) => {
